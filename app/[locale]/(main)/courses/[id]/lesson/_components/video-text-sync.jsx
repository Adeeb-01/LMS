...
export const VideoTextSync = ({ 
  ...
  const [conceptGaps, setConceptGaps] = useState([]);
  const videoRef = useRef(null);
  const lastUpdateTimeRef = useRef(0);

  const fetchConceptGaps = useCallback(async () => {
    if (!lessonId) return;
    const res = await getConceptGapSummary(lessonId);
    if (res.ok) {
      setConceptGaps(res.gaps);
    }
  }, [lessonId]);
  ...
  return (
    <VideoSyncContext.Provider value={{ activeBlockIndex, seekTo, handleTimeUpdate, videoRef, alignments, conceptGaps, refreshConceptGaps: fetchConceptGaps }}>
      {children}
