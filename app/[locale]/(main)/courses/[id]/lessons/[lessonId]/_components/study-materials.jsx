  ...
  const renderBlock = (block, index) => {
    ...
    switch (block.type) {
      case 'heading':
        ...
        return (
          <div {...commonProps} className={cn(commonProps.className, headingStyles[block.level] || headingStyles[3])}>
            <BadgeOverlay />
            {block.content}
          </div>
        );
      case 'list':
        return (
          <div {...commonProps} className={cn(commonProps.className, "flex gap-2 my-1 ml-4")}>
            <BadgeOverlay />
      ...
      default:
        return (
          <p {...commonProps} className={cn(commonProps.className, "text-sm leading-relaxed my-2 text-foreground/80")}>
            <BadgeOverlay />
            {block.content}
          </p>
        );
