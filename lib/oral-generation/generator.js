...
// Model candidates (most budget-friendly first for free tier)
...
const systemInstruction = `You are an expert university-level oral examination creator. 
...
${existingContext}
    ...
    while (retryCount <= MAX_RETRIES) {
        const modelName = MODEL_CANDIDATES[modelIndex] || MODEL_CANDIDATES[0];
        ...
        try {
            ...
            // Try next model if available and rate limited
            if ((isRateLimit || isOverloaded) && modelIndex < MODEL_CANDIDATES.length - 1) {
                console.warn(`[ORAL_GEN] Model ${modelName} overloaded, trying fallback...`);
                modelIndex++;
                continue;
            }
            ...
            if (isRateLimit && retryCount < MAX_RETRIES) {
                ...
                console.warn(`[ORAL_GEN_RETRY] Rate limit hit. Retrying in ${Math.round(delay/1000)}s (Attempt ${retryCount}/${MAX_RETRIES})`);
                await new Promise(resolve => setTimeout(resolve, delay));
                modelIndex = 0; // Reset to first model for retries
                continue;
            }
    ...
    // If we exit the loop without returning (max retries exceeded)
    return [];
}
