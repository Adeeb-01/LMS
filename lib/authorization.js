...
export async function assertInstructorOwnsModule(moduleId, userId, user = null) {
  ...
  if (user && isAdmin(user)) {
    ...
    return { module: courseModule, course: null };
  }
...
export async function assertInstructorOwnsLesson(lessonId, userId, user = null) {
  ...
  const courseModule = await Module.findOne({ lessonIds: lessonId })
    .select('course')
    .lean();
...
export async function verifyOwnsAllModules(moduleIds, userId, user = null) {
  ...
  // Filter out invalid ObjectIds (client may pass temporary IDs)
  const validModuleIds = moduleIds.filter(id => mongoose.Types.ObjectId.isValid(id));
  ...
  const modules = await Module.find({ _id: { $in: validModuleIds } })
    .select('course')
    .lean();
  
  if (modules.length !== validModuleIds.length) {
...
export async function verifyOwnsAllLessons(lessonIds, userId, user = null) {
  ...
  // Filter out invalid ObjectIds (client may pass temporary IDs)
  const validLessonIds = lessonIds.filter(id => mongoose.Types.ObjectId.isValid(id));
  ...
  const modules = await Module.find({ lessonIds: { $in: validLessonIds } })
